function [ image_stack, scriptV ] = load_syn_images( image_dir )
    %LOAD_SYN_IMAGES_COLOR read from directory image_dir all files with extension png
...
%   scriptV: light directions
    ...
    for i = 1:nfiles
        ...
        if image_stack == 0
            ...
            image_stack = zeros(h, w, nfiles, size(im, 3), 'uint8');
            V = zeros(nfiles, 3, 'double');
        end

        % add all channels
        for i_ch = 1:size(im,3)
            image_stack(:, :, i, i_ch) = im(:, :, i_ch);
        end
